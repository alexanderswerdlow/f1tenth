...
sfunxyauto(t, x, u, flag, ax, varargin
)
%
SFUNXYAUTO S
-
...
case 0
[sys, x0, str, ts] =
mdlInitializeSizes(ax, varargin{:}
);
...
This is
the XYGraph
block's LoadFcn and CopyFcn.  Initialize the block's
%
UserData such
that a
figure is
...
the XY
Graph figure
window's DeleteFcn.  The figure window is
%
being deleted, update
the XY
Graph block
's UserData to reflect the change.
%=============================================================================
%
...
XYLine = plot(0, 0);
ud.
XYXlabel = xlabel('X Axis');
ud.
XYYlabel = ylabel('Y Axis');
ud.
XYTitle = get(ud.XYAxes, 'Title');
ud.
XData = [];
ud.
YData = [];
...
% if
the block
isn't linked, issue a warning, and then set the callbacks
% for
the block
so that
it has
...
strcmp (get_param(block,
'LinkStatus'), 'none'),
warnmsg = sprintf(['The XY Graph Autoscale scope ''%s'
' should be replaced ' ...
...
length(callbacks),
if ~
strcmp (get_param(block, callbacks { i, 1 }
), callbacks{
i, 2
}),
