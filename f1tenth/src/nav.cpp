 ...
 *
 * This dummy subscribes for the goals set manually in rviz. The goals are then
 * wrapped in an action and send to path_control.
 */

class HighDummy {
 public:
  HighDummy(ros::NodeHandle &nh) : nh_(nh), pnh_("~"), client_("navigate_to_goal", true) {
 ...
 private:
  ...
  void doneCb(const actionlib::SimpleClientGoalState &state, const path_msgs::NavigateToGoalResultConstPtr &result) {
	...
	if (!result) {
	  ...
	  const char *status_names[] = {"OTHER_ERROR", "SUCCESS", "ABORTED", "OBSTACLE",
									"TIMEOUT", "LOST_PATH", "NO_PATH_FOUND"};
  ...
  void feedbackCb(const path_msgs::NavigateToGoalFeedbackConstPtr &feedback) {
	switch (feedback->status) {
	  case NavigateToGoalFeedback::STATUS_MOVING:ROS_INFO_THROTTLE(1, "Feedback: Moving");
		ROS_DEBUG_THROTTLE(1, "There are %zu obstacles sighted on the path.", feedback->obstacles_on_path.size());
		break;

	  case NavigateToGoalFeedback::STATUS_PATH_READY:ROS_INFO("Feedback: Path is ready.");
		break;

	  case NavigateToGoalFeedback::STATUS_OBSTACLE:ROS_WARN_THROTTLE(1, "Feedback: Collision.");
		break;

	  case NavigateToGoalFeedback::STATUS_NO_LOCAL_PLAN:ROS_WARN_THROTTLE(1, "Feedback: No local plan found.");
		break;

	  case NavigateToGoalFeedback::STATUS_REPLAN:ROS_WARN("Path is replaned.");
		break;

	  case NavigateToGoalFeedback::STATUS_REPLAN_FAILED:ROS_ERROR("Replan failed.");
		break;

	  default:ROS_ERROR("Feedback: Unknown status code %d", feedback->status);
		break;
	}
  }
  ...
  void goalCb(const geometry_msgs::PoseStampedConstPtr &pose) {
	...
	if (pose->header.frame_id != world_frame_) {
	  if (!tfl_.waitForTransform(world_frame_, pose->header.frame_id, pose->header.stamp, ros::Duration(10.0))) {
		ROS_ERROR_STREAM("cannot drive to goal, the transformation between " << world_frame_ << " and "
																			 << pose->header.frame_id << " is not known");
	  }
	...
	client_.sendGoal(goal, boost::bind(&HighDummy::doneCb, this, _1, _2), boost::bind(&HighDummy::activeCb, this),
