...
double t0 = 0, t1, s0 = 0, s1, vs0 = 0, vs1, as0 = 0, as1, d0 = 0, d1, vd0 = 0, vd1, ad0 = 0,
	ad1;              ////starting states for every replanning stage
...
int optimum_traj_no;
double global_velocity[(int) cl_traj_pt], global_omega[(int) cl_traj_pt], global_theta_x[(int) cl_traj_pt], global_x[(int) cl_traj_pt],
	global_y[(int) cl_traj_pt];
double exec_t_step;
...
void odom_scan_callback(const nav_msgs::Odometry::ConstPtr &odom_msg, const sensor_msgs::LaserScan::ConstPtr &scan_msg) {
  ...
  geometry_msgs::Vector3Stamped tf_twist_linear;
  geometry_msgs::Vector3Stamped temp_twist_linear;
  temp_twist_linear.header = odom_msg->header;
  temp_twist_linear.vector = odom_msg->twist.twist.linear;

  geometry_msgs::Vector3Stamped tf_twist_angular;
  geometry_msgs::Vector3Stamped temp_twist_angular;
  temp_twist_angular.header = odom_msg->header;
  temp_twist_angular.vector = odom_msg->twist.twist.angular;

  tf_listener->transformPose("map", ros::Time(0), temp_pose, "odom", tf_pose);
  tf_listener->transformVector("map", ros::Time(0), temp_twist_linear, "odom", tf_twist_linear);
  tf_listener->transformVector("map", ros::Time(0), temp_twist_angular, "odom", tf_twist_angular);

  pose_msg.header = odom_msg->header;                                                    /////header.seq
  pose_msg.pose.pose.position = tf_pose.pose.position;                            /////pose.pose.position.x
  pose_msg.pose.pose.orientation = tf_pose.pose.orientation;                    /////pose.pose.orientation.x   /////quaternion
  pose_msg.twist.twist.linear = tf_twist_linear.vector;                            /////twist.twist.linear.x
  pose_msg.twist.twist.angular = tf_twist_angular.vector;                        /////twist.twist.angular.x
...
void publish_vw(int x) {
  ...
  if (x < 0) {
	...
	if (!stop_replanning) {
	  ...
	  double query_index = (x / (double) set_loop_rate)
		  / exec_t_step;                   /////////// (x/loop_rate) will be between 2 timesteps of index*exec_t_step

	  double final_velocity = ((ceil(query_index) - query_index) * global_velocity[(int) floor(query_index)]
		  + (query_index - floor(query_index)) * global_velocity[(int) ceil(query_index)]) / (ceil(query_index) - floor(query_index));
	  double final_omega = ((ceil(query_index) - query_index) * global_omega[(int) floor(query_index)]
		  + (query_index - floor(query_index)) * global_omega[(int) ceil(query_index)]) / (ceil(query_index) - floor(query_index));
...
void get_global_waypoints() {
  cl_way_pt[0] << 0, 0;
  cl_way_pt[1] << 7, 0;                                                //////0,0; 36,0; 36,-45;
  cl_way_pt[2] << 17, 0;
  // cl_way_pt[3] <<  10, 30;
  // cl_way_pt[4] <<  40, 30;
  // cl_way_pt[5] <<  60, 10;
  ...
  while (i < (ways - 2)) {
	...
	//if(traj_pt==1)
	{
	  ...
	  if ((temp_pos1 - temp_pos).norm() > 0.2) {
		...
		for (int x = 0; x < cl_straight_scale - 1;
			 x++)                                                            ///////////199 so that no point are exactly same, ds should be greater than zero always
		{
...
//	double robot_current_s,robot_current_d;
  ...
  tf::Quaternion q(pose_msg.pose.pose.orientation.x,
  ...
  cl_vector << (cl_way_traj[prev_cl_pt + 1](0) - cl_way_traj[prev_cl_pt - 1](0)), (cl_way_traj[prev_cl_pt + 1](1)
	  - cl_way_traj[prev_cl_pt - 1](1)), 0;
  ...
  s_dot =
	  (sqrt(pow(pose_msg.twist.twist.linear.x, 2) + pow(pose_msg.twist.twist.linear.y, 2))) * cos(theta_r[prev_cl_pt] - robot_current_yaw);
  d_dot =
	  -(sqrt(pow(pose_msg.twist.twist.linear.x, 2) + pow(pose_msg.twist.twist.linear.y, 2))) * sin(theta_r[prev_cl_pt] - robot_current_yaw);

  s_dot_dot = (sqrt(pow(accn_msg.linear_acceleration.x, 2) + pow(accn_msg.linear_acceleration.y, 2)))
	  * cos(theta_r[prev_cl_pt] - robot_current_yaw);
  d_dot_dot = -(sqrt(pow(accn_msg.linear_acceleration.x, 2) + pow(accn_msg.linear_acceleration.y, 2)))
	  * sin(theta_r[prev_cl_pt] - robot_current_yaw);
...
//take only obstacles inside 10m distance
  ...
  for (int x = 0; x < obs_no + 1; x++) {
	...
	cl_vector << (cl_way_traj[closest_pt_index + 1](0) - cl_way_traj[closest_pt_index - 1](0)), (cl_way_traj[closest_pt_index + 1](1)
		- cl_way_traj[closest_pt_index - 1](1)), 0;
	cl2pose_vector << (obs_global_center[x](0) - cl_way_traj[closest_pt_index](0)), (obs_global_center[x](1)
		- cl_way_traj[closest_pt_index](1)), 0;
	...
	cout << "obstacle(" << x + 1 << ") s,d = " << obs[x].center_s << "," << obs[x].center_d << " radius = " << obs[x].clearing_radius
		 << " and the closest point on center line = " << cl_way_traj[closest_pt_index](0) << "," << cl_way_traj[closest_pt_index](1)
		 << endl;
...
/// give collision_status 0(no collision) & 1(collision_detected)
  ...
  for (int i = 0; i < sd_traj::traj_counter; i++) {
	...
	for (int i1 = 0; i1 < t_div; i1++) {
	  ...
	  temp_st = traj[i].a_mat(0, 0) * pow(temp_T, 5) + traj[i].a_mat(1, 0) * pow(temp_T, 4) + traj[i].a_mat(2, 0) * pow(temp_T, 3)
		  + traj[i].a_mat(3, 0) * pow(temp_T, 2) +
		  traj[i].a_mat(4, 0) * temp_T + traj[i].a_mat(5, 0);
	  ///// s(T)

	  temp_ds = traj[i].b_mat(0, 0) * pow(temp_st, 5) + traj[i].b_mat(1, 0) * pow(temp_st, 4) + traj[i].b_mat(2, 0) * pow(temp_st, 3)
		  + traj[i].b_mat(3, 0) * pow(temp_st, 2) +
	  ...
	  for (int j = 0; j < obs_no + 1; j++) {
		...
		temp_D = pow(fabs(obs[j].center_s - temp_st), 2)
			+ pow(fabs(obs[j].center_d - temp_ds), 2);      //// D^2 > rc^2 for every obstacle on the trajpoint selected
...
///calculate cost for collision_status-0 and simultaneously sort sd_traj ascendingly
  ...
  for (int i = 0; i < sd_traj::traj_counter; i++) {
	...
	if ((traj[i].collision_status == 0) || obs_no < 0) {
	  ...
	  double del_t = traj[i].t1 - traj[i].t0, del_s = traj[i].c_a_mat(1, 0) - s0,
		  v = traj[i].c_a_mat(3, 0);                        //////del_t=t1-t0, del_s=s1-s0 of that particular traj

	  double cost_jerklon =
		  720 * (pow(traj[i].a_mat(0, 0), 2) * pow(del_t, 5)) + (720 * (traj[i].a_mat(0, 0) * traj[i].a_mat(1, 0)) * pow(del_t, 4)) +
			  (240 * (traj[i].a_mat(0, 0) * traj[i].a_mat(2, 0)) + 192 * (pow(traj[i].a_mat(1, 0), 2))) * pow(del_t, 3)
			  + (144 * (traj[i].a_mat(1, 0) * traj[i].a_mat(2, 0)) * pow(del_t, 2)) + +(36 * (pow(traj[i].a_mat(2, 0), 2) * del_t));

	  cost_lon = kj_lon * cost_jerklon + kt_lon * del_t + ks_lon * pow(del_s, 2) + kv_lon * v;

	  double cost_jerklat =
		  720 * (pow(traj[i].b_mat(0, 0), 2) * pow(del_s, 5)) + (720 * (traj[i].b_mat(0, 0) * traj[i].b_mat(1, 0)) * pow(del_s, 4)) +
			  (240 * (traj[i].b_mat(0, 0) * traj[i].b_mat(2, 0)) + 192 * (pow(traj[i].b_mat(1, 0), 2))) * pow(del_s, 3)
			  + (144 * (traj[i].b_mat(1, 0) * traj[i].b_mat(2, 0)) * pow(del_s, 2)) + +(36 * (pow(traj[i].b_mat(2, 0), 2) * del_s));
  ...
  std::sort(traj,
			traj + sd_traj::traj_counter,
			sort_low_cost);                                    ////sort all trajectories according to minimum cost
...
////start from lowest-cost traj and calculate Kmax and check till condition is satisfied
  ...
  if (!stop_replanning) {
	...
	for (int i = 0; i < sd_traj::traj_counter; i++) {
	  ...
	  double s[cl_pt_indices], d[cl_pt_indices], s_dot[cl_pt_indices], d_dash[cl_pt_indices], d_dot[cl_pt_indices],
		  s_dot_dot[cl_pt_indices], d_dash_dash[cl_pt_indices], d_dot_dot[cl_pt_indices];
	  ...
	  for (int k = 0; k < cl_pt_indices; k++) {
		s[k] = traj[i].a_mat(0, 0) * pow(temp_T, 5) + traj[i].a_mat(1, 0) * pow(temp_T, 4) + traj[i].a_mat(2, 0) * pow(temp_T, 3)
			+ traj[i].a_mat(3, 0) * pow(temp_T, 2) +
			traj[i].a_mat(4, 0) * temp_T + traj[i].a_mat(5, 0);
		d[k] = traj[i].b_mat(0, 0) * pow(s[k], 5) + traj[i].b_mat(1, 0) * pow(s[k], 4) + traj[i].b_mat(2, 0) * pow(s[k], 3)
			+ traj[i].b_mat(3, 0) * pow(s[k], 2) +
			traj[i].b_mat(4, 0) * s[k] + traj[i].b_mat(5, 0);

		s_dot[k] =
			5 * traj[i].a_mat(0, 0) * pow(temp_T, 4) + 4 * traj[i].a_mat(1, 0) * pow(temp_T, 3) + 3 * traj[i].a_mat(2, 0) * pow(temp_T, 2)
				+ 2 * traj[i].a_mat(3, 0) * temp_T +
				traj[i].a_mat(4, 0);
		d_dash[k] = 5 * traj[i].b_mat(0, 0) * pow(s[k], 4) + 4 * traj[i].b_mat(1, 0) * pow(s[k], 3) + 3 * traj[i].b_mat(2, 0) * pow(s[k], 2)
			+ 2 * traj[i].b_mat(3, 0) * s[k] +
			traj[i].b_mat(4, 0);

		d_dot[k] = s_dot[k] * d_dash[k];

		s_dot_dot[k] =
			20 * traj[i].a_mat(0, 0) * pow(temp_T, 3) + 12 * traj[i].a_mat(1, 0) * pow(temp_T, 2) + 6 * traj[i].a_mat(2, 0) * temp_T
				+ 2 * traj[i].a_mat(3, 0);
		d_dash_dash[k] = 20 * traj[i].b_mat(0, 0) * pow(s[k], 3) + 12 * traj[i].b_mat(1, 0) * pow(s[k], 2) + 6 * traj[i].b_mat(2, 0) * s[k]
			+ 2 * traj[i].b_mat(3, 0);

		d_dot_dot[k] = d_dash_dash[k] * pow(s_dot[k], 2) + d_dash[k] * s_dot_dot[k];

		global_theta_x[k] =
			theta_r[prev_cl_pt + k] + atan(d_dash[k] / (1 - kappa[prev_cl_pt + k] * d[k]));                    /////////////////// omega
		...
		if (k > 0) {
		  ...
		  temp_b = ((d_dash_dash[k] + temp_a * tan(del_theta[k])) * (pow(cos(del_theta[k]), 2) / (1 - kappa[prev_cl_pt + k] * d[k])))
			  + kappa[prev_cl_pt + k];
	  ...
	  if (traj[i].max_curvature_status == 0) {
		optimum_traj_no = i;
		cout << "optimum_traj= " << optimum_traj_no << ", deviation= " << traj[optimum_traj_no].c_b_mat(1, 0) << ", velocity= "
			 << traj[optimum_traj_no].c_a_mat(3, 0) << "\n" << endl;
...
int main(int argc, char **argv) {
  ...
  velocity_pub = n.advertise<geometry_msgs::Twist>("/cmd_vel", 0);
