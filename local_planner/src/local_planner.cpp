...
void odom_scan_callback(const nav_msgs::Odometry::ConstPtr& odom_msg, const sensor_msgs::LaserScan::ConstPtr& scan_msg)
{
  ...
  try
  {
	...
	geometry_msgs::PoseStamped odom_new;
	odom_new.header = odom_msg->header;
	odom_new.pose = odom_msg->pose.pose;

	geometry_msgs::TwistStamped odom_goal1;
	geometry_msgs::TwistStamped odom_new1;
	odom_new.header = odom_msg->header;
	odom_new.pose = odom_msg->twist;
	tf_listener->transformPose("map", ros::Time(0), odom_new, "odom", odom_goal);


	pose_msg.header = odom_msg->header;													/////header.seq
	pose_msg.pose.pose.position = odom_goal.pose.position;							/////pose.pose.position.x
	pose_msg.pose.pose.orientation = odom_goal.pose.orientation;					/////pose.pose.orientation.x   /////quaternion
