...
u_result SocketAddress::getRawAddress(_u8 *buffer, size_t bufferSize) const {
  switch (getAddressType()) {
	case ADDRESS_TYPE_INET:if (bufferSize < sizeof(in_addr::s_addr)) return RESULT_INSUFFICIENT_MEMORY;
	  ...
	  break;
	case ADDRESS_TYPE_INET6:if (bufferSize < sizeof(in6_addr::s6_addr)) return RESULT_INSUFFICIENT_MEMORY;
	  memcpy(buffer,
...
class _single_thread StreamSocketImpl : public StreamSocket {
 public:
  ...
  virtual u_result connect(const SocketAddress &pairAddress) {
	...
	switch (errno) {
	  case EAFNOSUPPORT:return RESULT_OPERATION_NOT_SUPPORT;
#if 0
	  case EINPROGRESS:
		  return RESULT_OK; //treat async connection as good status
#endif
	  case ETIMEDOUT:return RESULT_OPERATION_TIMEOUT;
	  default:return RESULT_OPERATION_FAIL;
	}
  }
  ...
  virtual StreamSocket *accept(SocketAddress *pairAddress) {
	...
	int pair_socket = ::accept(_socket_fd,
  ...
  virtual u_result shutdown(socket_direction_mask mask) {
	...
	switch (mask) {
	  case SOCKET_DIR_RD:shutdw_opt = SHUT_RD;
		break;
	  case SOCKET_DIR_WR:shutdw_opt = SHUT_WR;
		break;
	  case SOCKET_DIR_BOTH:
	  default:shutdw_opt = SHUT_RDWR;
	}
  ...
  virtual u_result sendTo(const SocketAddress &target, const void *buffer, size_t len) {
	...
	if (ans != (size_t) -1) {
	  ...
	  switch (errno) {
		...
		case EMSGSIZE:return RESULT_INVALID_DATA;
		default:return RESULT_OPERATION_FAIL;
	  }
...
static inline int _socketHalFamilyToOSFamily(SocketBase::socket_family_t family) {
  switch (family) {
	...
	case SocketBase::SOCKET_FAMILY_RAW:return AF_PACKET;
	default:assert(!"should not reach here");
