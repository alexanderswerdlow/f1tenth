...
static inline int _halAddrTypeToOSType(SocketAddress::address_type_t type) {
  switch (type) {
	case SocketAddress::ADDRESS_TYPE_INET:return AF_INET;
	case SocketAddress::ADDRESS_TYPE_INET6:return AF_INET6;
	case SocketAddress::ADDRESS_TYPE_UNSPEC:return AF_UNSPEC;

	default:assert(!"should not reach here");
	  return AF_UNSPEC;
  }
}
...
SocketAddress::SocketAddress(void *platform_data)
	: _platform_data(platform_data) { _checkWSAStartup(); }
...
SocketAddress::address_type_t SocketAddress::getAddressType() const {
  switch (reinterpret_cast<const sockaddr_storage *>(_platform_data)->ss_family) {
	case AF_INET:return ADDRESS_TYPE_INET;
	case AF_INET6:return ADDRESS_TYPE_INET6;
	default:assert(!"should not reach here");
...
int SocketAddress::getPort() const {
  switch (getAddressType()) {
	case ADDRESS_TYPE_INET:return (int) ntohs(reinterpret_cast<const sockaddr_in *>(_platform_data)->sin_port);
	case ADDRESS_TYPE_INET6:return (int) ntohs(reinterpret_cast<const sockaddr_in6 *>(_platform_data)->sin6_port);
	default:return 0;
  }
}
...
u_result SocketAddress::setPort(int port) {
  switch (getAddressType()) {
	case ADDRESS_TYPE_INET:reinterpret_cast<sockaddr_in *>(_platform_data)->sin_port = htons((short) port);
	  break;
	case ADDRESS_TYPE_INET6:reinterpret_cast<sockaddr_in6 *>(_platform_data)->sin6_port = htons((short) port);
	  break;
	default:return RESULT_OPERATION_FAIL;
  }
  return RESULT_OK;
}
...
u_result SocketAddress::setAddressFromString(const char *address_string, SocketAddress::address_type_t type) {
  ...
  switch (type) {
	case ADDRESS_TYPE_INET:reinterpret_cast<sockaddr_storage *>(_platform_data)->ss_family = AF_INET;
	...
	default:return RESULT_INVALID_DATA;
...
u_result SocketAddress::getRawAddress(_u8 *buffer, size_t bufferSize) const {
  switch (getAddressType()) {
	case ADDRESS_TYPE_INET:
	  if (bufferSize < sizeof(reinterpret_cast<const sockaddr_in *>(_platform_data)->sin_addr.s_addr))
		return RESULT_INSUFFICIENT_MEMORY;

	  memcpy(buffer,
			 &reinterpret_cast<const sockaddr_in *>(_platform_data)->sin_addr.s_addr,
			 sizeof(reinterpret_cast<const sockaddr_in *>(_platform_data)->sin_addr.s_addr));

	  break;
	case ADDRESS_TYPE_INET6:
	  if (bufferSize < sizeof(reinterpret_cast<const sockaddr_in6 *>(_platform_data)->sin6_addr.s6_addr))
		return RESULT_INSUFFICIENT_MEMORY;
	  memcpy(buffer,
			 reinterpret_cast<const sockaddr_in6 *>(_platform_data)->sin6_addr.s6_addr,
			 sizeof(reinterpret_cast<const sockaddr_in6 *>(_platform_data)->sin6_addr.s6_addr));
...
class _single_thread StreamSocketImpl
:
public StreamSocket {
public:

StreamSocketImpl(SOCKET
fd)
:
_socket_fd(fd)
	{
...
virtual u_result connect(const SocketAddress &pairAddress) {
  ...
  switch (WSAGetLastError()) {
	case WSAEAFNOSUPPORT:return RESULT_OPERATION_NOT_SUPPORT;
#if 0
	case EINPROGRESS:
		return RESULT_OK; //treat async connection as good status
#endif
	case WSAETIMEDOUT:return RESULT_OPERATION_TIMEOUT;
	default:return RESULT_OPERATION_FAIL;
  }
}
...
virtual StreamSocket *accept(SocketAddress *pairAddress) {
  ...
  SOCKET
  pair_socket = ::accept(_socket_fd,
						 pairAddress ? reinterpret_cast<struct sockaddr *>(const_cast<void *>(pairAddress->getPlatformData())) : NULL,
						 &addrsize);
...
virtual u_result shutdown(socket_direction_mask mask) {
  ...
  switch (mask) {
	case SOCKET_DIR_RD:shutdw_opt = SD_RECEIVE;
	  break;
	case SOCKET_DIR_WR:shutdw_opt = SD_SEND;
	  break;
	case SOCKET_DIR_BOTH:
	default:shutdw_opt = SD_BOTH;
  }
...
virtual u_result enableKeepAlive(bool enable) {
  ...
  return ::setsockopt(_socket_fd, SOL_SOCKET, SO_KEEPALIVE, (const char *) &bool_true, (int) sizeof(bool_true)) ? RESULT_OPERATION_FAIL
																												: RESULT_OK;
}

virtual u_result enableNoDelay(bool enable) {
  int bool_true = enable ? 1 : 0;
  return ::setsockopt(_socket_fd, IPPROTO_TCP, TCP_NODELAY, (const char *) &bool_true, (int) sizeof(bool_true)) ? RESULT_OPERATION_FAIL
																												: RESULT_OK;
}
...
class _single_thread DGramSocketImpl
:
public DGramSocket {
public:

DGramSocketImpl(SOCKET
fd)
:
_socket_fd(fd)
	{
...
virtual u_result getLocalAddress(SocketAddress &localaddr) {
  struct sockaddr
	  *addr = reinterpret_cast<struct sockaddr *>(const_cast<void *>((localaddr.getPlatformData()))); //donnot do this at home...
  assert(addr);
...
virtual u_result sendTo(const SocketAddress &target, const void *buffer, size_t len) {
  ...
  if (ans != SOCKET_ERROR) {
	...
	switch (WSAGetLastError()) {
	  case WSAETIMEDOUT:return RESULT_OPERATION_TIMEOUT;
	  case WSAEMSGSIZE:return RESULT_INVALID_DATA;
	  default:return RESULT_OPERATION_FAIL;
	}
  }
...
static inline int _socketHalFamilyToOSFamily(SocketBase::socket_family_t family) {
  switch (family) {
	case SocketBase::SOCKET_FAMILY_INET:return AF_INET;
	case SocketBase::SOCKET_FAMILY_INET6:return AF_INET6;
	case SocketBase::SOCKET_FAMILY_RAW:return AF_UNSPEC; //win32 doesn't support RAW Packet
	default:assert(!"should not reach here");
